...
void multi_codispatch(iocschedule *S, void *arg) //协程，分离器
{
	...
	if (lcormsg->_evcallback != NULL)
	{
		(*(ievent_callback)lcormsg->_evcallback)(lcormsg->_evid, lcormsg->_evarg);
	}
}
...
void multi_dispatch(iocsystem *lsys, iocmultithread_t *lth)
{
	...
	while (lsys->_status != IOC_SYS_STOPING && lsys->_status != IOC_SYS_NOMAL)
	{
		...
		else
		{
			...
			if (lcormsg != NULL)
			{
				lcormsg->_evid  = lmsg->_evt->_evt_tye;
				lcormsg->_evcallback = lmsg->_evt->_evt_call;
				lcormsg->_evarg = lmsg->_evt_arg;
